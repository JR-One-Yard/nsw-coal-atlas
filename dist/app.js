...
function select(id,open=true){const f=[...data.seams,...data.mines,...data.destinations].find(x=>x.id===id);if(!f)return false;if(open){$('strataPanel').hidden=true;$('detailBody').hidden=false;$('detailHeading').textContent='Details';if(state.seam!=='all'&&f.id!==state.seam&&!f.seams?.includes(state.seam))$('seamSelect').value='all';if(!f.mesh)$('showMines').checked=true;}selected=id;if(open)openPanel('details');$('featureSelect').value=id;
 ...
 detail+=`<details><summary>Source evidence</summary><ul>${srcLinks(f.sources)}</ul></details>`;$('detailBody').innerHTML=detail;$('detailBody').querySelectorAll('[data-feature]').forEach(b=>b.onclick=()=>select(b.dataset.feature));update();return true;}
...
function reset(){geology?.reset();for(const [k,v]of Object.entries(DEFAULTS)){const e=$(k==='seam'?'seamSelect':k);if(e){if(typeof v==='boolean')e.checked=v;else e.value=v;}}selected=null;update();chapter(0);}
function setupUI(){

...
let geology,industry,lastRouteScale=null;
const internal={showRoutes:false};
...
function initScene(){
 scene=new THREE.Scene();scene.background=new THREE.Color('#0b1e2a');scene.fog=new THREE.FogExp2('#0b1e2a',.00135);
 camera=new THREE.PerspectiveCamera(43,1,.1,1600);
 ...
 controls=new OrbitControls(camera,renderer.domElement);controls.enableDamping=!matchMedia('(prefers-reduced-motion: reduce)').matches;controls.dampingFactor=.08;controls.minDistance=5;controls.maxDistance=440;controls.maxPolarAngle=Math.PI*.78;controls.addEventListener('start',()=>{pause();transition=null;});
 ...
 for(const f of [...data.mines,...data.destinations]){
  const col=colourFor(f.tags),g=new THREE.Group();const mesh=new THREE.Mesh(f.kind==='mine'||f.kind==='historic'?sphere:diamond,new THREE.MeshStandardMaterial({color:col,emissive:col,emissiveIntensity:.65,roughness:.5}));mesh.userData={kind:'feature',id:f.id};g.add(mesh);clickables.push(mesh);
  const ring=new THREE.Mesh(ringGeo,new THREE.MeshBasicMaterial({color:col,side:THREE.DoubleSide,transparent:true,opacity:.6}));ring.rotation.x=-Math.PI/2;ring.position.y=-.3;g.add(ring);scene.add(g);
 ...
 const borePositions=[];for(const b of data.bores){const p=b.position;const dep=Number.isFinite(b.depth)&&b.depth>0?b.depth:0;borePositions.push(...p,p[0],p[1]-dep/1000,p[2]);}
 boreGroup=new THREE.LineSegments(makeGeo(borePositions),new THREE.LineBasicMaterial({color:'#95b6d0',transparent:true,opacity:.3,clippingPlanes:[clipPlane]}));scene.add(boreGroup);
 for(const p of data.places){const el=document.createElement('span');el.className='map-label'+(p.major?' city':'');el.textContent=p.name;$('labels').append(el);mapLabels.push({el,data:p});}
...
function update(){
 state={...stateFromControls(document),...internal};terrainMesh.scale.y=state.exaggeration;terrainMesh.material.opacity=state.opacity/100;terrainMesh.material.depthWrite=state.opacity>=98;terrainMesh.visible=state.opacity>0;
 ...
 rebuildRoutes();for(const [id,r]of routeObjects){r.group.visible=state.showRoutes&&rs.has(id);}
...
function select(id,open=true){const f=[...data.seams,...data.mines,...data.destinations].find(x=>x.id===id);if(!f)return false;if(open){$('strataPanel').hidden=true;$('detailBody').hidden=false;$('detailHeading').textContent='Details';if(state.seam!=='all'&&f.id!==state.seam&&!f.seams?.includes(state.seam))$('seamSelect').value='all';if(!f.mesh)$('showMines').checked=true;}selected=id;if(open)openPanel('details');$('featureSelect').value=id;
 ...
 else{detail+=`<dl><dt>Location</dt><dd>${f.lat.toFixed(4)}°, ${f.lon.toFixed(4)}°</dd><dt>Evidence</dt><dd>${escape(f.confidence)}</dd>${f.method?`<dt>Method</dt><dd>${escape(f.method)}</dd>`:''}</dl>${f.status?`<p class="micro">${escape(f.status)}</p>`:''}`;const routes=data.routes.filter(r=>r.fromId===id||r.toId===id);if(routes.length){detail+='<h3>Where the coal goes</h3>'+routes.map(r=>{const a=data.mines.find(m=>m.id===r.fromId),b=data.destinations.find(d=>d.id===r.toId);return `<p class="micro">${escape(a.name)} → ${escape(b.name)}<br>${escape(r.mode)}</p>`;}).join('')+`<button id="showConnection">${internal.showRoutes?'Hide connections':'Show connections'}</button><p class="micro">Schematic links; no actual route alignment or shipment volume implied.</p>`;}}
 detail+=`<details><summary>Source evidence</summary><ul>${srcLinks(f.sources)}</ul></details>`;$('detailBody').innerHTML=detail;$('detailBody').querySelectorAll('[data-feature]').forEach(b=>b.onclick=()=>select(b.dataset.feature));if($('showConnection'))$('showConnection').onclick=()=>{$('seamSelect').value='all';internal.showRoutes=!internal.showRoutes;$('showConnection').textContent=internal.showRoutes?'Hide connections':'Show connections';$('showMines').checked=true;update();if(internal.showRoutes)focusFeature(id);};update();return true;}
...
function regionNotes(){if($('regionSelect').value==='all'){$('strataPanel').hidden=true;$('detailBody').hidden=false;$('detailHeading').textContent='Sydney Basin';$('detailBody').innerHTML='<h3>Illawarra to the Hunter</h3><p>The atlas follows five coalfield regions along the NSW coast. Choose a region to see its terrain, coal layers and mines.</p><p>The underground surfaces are illustrative. They do not form a continuous, verified seam model.</p>';openPanel('details');return;}const c=data.chapters[currentChapter];$('strataPanel').hidden=true;$('detailBody').hidden=false;$('detailHeading').textContent=c.name;$('detailBody').innerHTML=`<h3>${escape(c.title)}</h3><p>${escape(regionCopy[currentChapter])}</p><button id="regionSeam">Read about ${escape(data.seams.find(s=>s.id===c.seam).name)}</button>`;$('regionSeam').onclick=()=>select(c.seam);openPanel('details');}
...
function overview(){pause();closePanels();$('regionSelect').value='all';transition={start:performance.now(),from:camera.position.clone(),to:new THREE.Vector3(160,170,235),targetFrom:controls.target.clone(),targetTo:new THREE.Vector3(0,0,0)};}
...
function reset(){internal.showRoutes=false;geology?.reset();for(const [k,v]of Object.entries(DEFAULTS)){const e=$(k==='seam'?'seamSelect':k);if(e){if(typeof v==='boolean')e.checked=v;else e.value=v;}}selected=null;update();chapter(0);}
function setupUI(){
 $('regionSelect').innerHTML=data.chapters.map((c,i)=>`<option value="${i}">${escape(c.name)}</option>`).join('')+'<option value="all">Whole basin</option>';$('regionSelect').onchange=e=>e.target.value==='all'?overview():chapter(+e.target.value);
 ...
 $('sourceBody').innerHTML=`<p>This atlas connects the underground geology with mines and coal uses along the NSW coast. It is an educational reconstruction, not a survey or resource model.</p><h3>What is measured, and what is reconstructed?</h3><ul><li><strong>Terrain:</strong> sampled public elevation data. Horizontal dimensions are kilometres; height and depth use the same selectable vertical exaggeration.</li><li><strong>Mine markers:</strong> GSNSW point locations for all 15 coal records returned within the selected corridor, plus an approximate historical Birchgrove location. Registry coverage is not a complete inventory of all current and historical mines.</li><li><strong>Underground layers:</strong> smooth conceptual sheets using published seam order and regional/site depth references. Footprints and dip are illustrative. Sydney and Hunter sheets represent formations, not individual mapped seams. No faults or mined-out volumes have been invented.</li><li><strong>Boreholes:</strong> ${data.boreDisplayed.toLocaleString()} spatially thinned points from ${data.boreTotal.toLocaleString()} corridor records. Lines use recorded total drilled depth; straight vertical traces are schematic. They do not show coal intercepts and were not used to fit the seams.</li><li><strong>Coal use:</strong> classifications describe documented uses near example mines. Quality and use can vary within a seam.</li><li><strong>Transport:</strong> schematic supply links. Links do not represent actual routes, tonnage or current shipments.</li></ul><h3>Scale and gaps</h3><p>The full corridor is regional in scale. Thin seams would be nearly invisible at true scale, so the display uses coloured surfaces and selectable vertical exaggeration. Sheet edges are model windows, not resource boundaries. The Central Coast gap is left without an invented seam connection. No continuous, verified seam-level 3D model was obtained.</p><h3>Downloads</h3><p><a href="./downloads/NSW-Coal-Atlas.blend" download>Download the editable Blender scene</a>. The full 150-second camera tour and source notes are embedded in the file. Toggle coal-use collections in Blender; mixed-use belongs under either filter.</p><p><a href="./downloads/NSW-Coal-Flythrough.mp4" download>Download the recorded flythrough (MP4)</a> · Silent recording of the complete 150-second interactive tour, including setup.</p><h3>Sources · snapshot 11 September 2026</h3><ol>${data.sources.map(s=>`<li><a href="${escape(s.url)}" target="_blank" rel="noopener">${escape(s.title)}</a><br>${escape(s.note)}</li>`).join('')}</ol><p><a href="./data/atlas.json" download>Download the model and provenance (JSON)</a> · <a href="./data/terrain.json" download>Download terrain samples</a> · <a href="./data/terrain-corrections.json" download>Reviewed terrain corrections and sources</a></p>`;
...
function animate(t){if($('mapPage').hidden||document.hidden){last=t;requestAnimationFrame(animate);return;}const dt=Math.min(.1,(t-last)/1000);last=t;frameCount++;
 ...
 controls.update();
 for(const p of mapLabels){const v=new THREE.Vector3(p.data.position[0],p.data.position[1]*state.exaggeration+1.3,p.data.position[2]);const dist=camera.position.distanceTo(v);v.project(camera);const show=state.showLabels&&v.z<1&&v.z>-1&&Math.abs(v.x)<.96&&Math.abs(v.y)<.92&&(p.data.major||dist<105)&&p.data.position[0]<=clipPlane.constant;p.el.hidden=!show;if(show){p.el.style.left=(v.x*.5+.5)*view.clientWidth+'px';p.el.style.top=(-v.y*.5+.5)*view.clientHeight+'px';}}
...
function registerTools(){
 ...
 window.coalAtlas={focusFeature,pause,getState:()=>({ready:true,state:{...state},selected,playing,progress,chapter:currentChapter,visibleSeams:visibleSeams(data,state).map(s=>s.id),visibleMines:visibleMines(data,state).map(s=>s.id),visibleRoutes:visibleRoutes(data,state).map(s=>s.id),frameCount,camera:camera.position.toArray(),target:controls.target.toArray(),triangles:renderer.info.render.triangles,renderedSeams:[...seamObjects].filter(([id,o])=>o.mesh.visible).map(([id])=>id),renderedMines:[...featureObjects].filter(([id,o])=>o.group.visible).map(([id])=>id),renderedRoutes:[...routeObjects].filter(([id,o])=>o.group.visible).map(([id])=>id),boreVisible:boreGroup.visible,groundOpacity:terrainMesh.material.opacity,clipX:clipPlane.constant}),select,seek,chapter,overview,reset};
...
try{
 const responses=await Promise.all([fetch('./data/atlas.json'),fetch('./data/terrain.json')]);if(responses.some(r=>!r.ok))throw new Error('The local dataset could not be loaded.');[data,terrain]=await Promise.all(responses.map(r=>r.json()));
 initScene();setupUI();update();chapter(0);select('bulli',false);registerTools();requestAnimationFrame(animate);
 ...
 industry=await initIndustry({data,select,focusFeature,pause});

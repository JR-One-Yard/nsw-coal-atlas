...
export async function initIndustry({data,select,focusFeature,pause}){
  ...
  function render(){
    ...
    $('mineRows').innerHTML=filtered.length?filtered.map(r=>`<tr class="${selected===r.id?'selected':''}"><td><button data-mine="${r.id}">${esc(r.name)}</button></td><td>${esc(r.region)}</td><td>${esc(r.operator??'Not verified')}</td><td>${product(r)}</td></tr>`).join(''):'<tr><td colspan="4">No mines match. Clear the search or choose a different filter.</td></tr>';

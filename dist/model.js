...
export function stateFromControls(root){return {...Object.fromEntries(['met','thermal','unknown','showMines','showRoutes','showBores','showLabels'].map(id=>[id,root.getElementById(id).checked])),...Object.fromEntries(['opacity','exaggeration','slice'].map(id=>[id,+root.getElementById(id).value])),seam:root.getElementById('seamSelect').value};}

...
export function stateFromControls(root){return {...DEFAULTS,...Object.fromEntries(['showMines','showLabels'].map(id=>[id,root.getElementById(id).checked])),...Object.fromEntries(['opacity','exaggeration'].map(id=>[id,+root.getElementById(id).value])),seam:root.getElementById('seamSelect').value};}
